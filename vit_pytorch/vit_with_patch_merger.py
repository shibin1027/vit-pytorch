...
class PatchMerger(nn.Module):
    def __init__(self, dim, num_tokens_out):
        ...
        self.queries = nn.Parameter(torch.randn(num_tokens_out, dim))  # m,d

    def forward(self, x):
        # x: b,n,d
        x = self.norm(x)
        sim = torch.matmul(self.queries, x.transpose(-1, -2)) * self.scale  # m,d * b,d,n -> b,m,n
        attn = sim.softmax(dim = -1)  # b,m,n
        return torch.matmul(attn, x)  # b,m,n * b,n,d -> b,m,d
